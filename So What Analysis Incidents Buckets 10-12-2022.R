...
s = fread(paste0(dataPath,'incident.csv'), stringsAsFactors = TRUE,strip.white=TRUE)
w<- read.csv("C:/Users/OITCOWasimB/OneDrive - Department of Veterans Affairs/Desktop/parentTTR.csv")
...
colnames(w)
w %>%
  inner_join(d_all %>% select(Incident_Ticket,Primary_SystemApplication_Affected,ttr), by = c("Parent.Incident" = 'Incident_Ticket'))
w %>% select(parentCulpable,Parent.Incident)%>%
  group_by(parentCulpable)%>% summarise(N=n())%>%arrange(desc(N))
w %>% 
  filter(ttr != "NA")%>%
  filter(parentCulpable == "VBMS")%>% 
  select(Parent.Incident,systemShort,parentCulpable,ttr)%>%
  group_by(parentCulpable,systemShort)%>%summarize(N = n(),MTTR=mean(ttr),MedTTR=median(ttr),TTR=sum(ttr)) %>% 
  mutate(p = (N/256)%>%round(2)*100)%>%arrange(desc(MedTTR))%>%print()%>%
  write.csv('p6.csv', row.names = F)
w %>% 
    filter(ttr != "NA")%>%
    filter(parentCulpable == "SSOi")%>% 
    select(Parent.Incident,systemShort,parentCulpable,ttr)%>% 
    group_by(parentCulpable,systemShort)%>%summarize(N = n(),MTTR=mean(ttr),MedTTR=median(ttr),TTR=sum(ttr)) %>% 
    mutate(p = (N/79)%>%round(2)*100)%>%arrange(desc(MedTTR))%>%print()%>%
  write.csv('p7.csv', row.names = F)
w%>% filter(Parent.Incident == "INC23856850")%>%
  select(Number,parentCulpable,Affected.CI,systemShort, Parent.Child,Parent.Incident,ttr  )%>% 
  group_by(parentCulpable,systemShort)%>%
  summarize(N = n(),TTR=sum(ttr)) %>% 
  mutate(p = prop.table(N)*100)%>%
  mutate(CHTTIME=TTR/N)%>%arrange(desc(N))
w%>% filter(Parent.Incident == "INC24323559")%>%
  select(Number,parentCulpable,systemShort,Affected.CI, Parent.Child,Parent.Incident,ttr)%>% 
  group_by(parentCulpable,systemShort)%>%
  summarize(N = n(),TTR=sum(ttr)) %>% 
  mutate(p = prop.table(N)*100)%>%
  mutate(CHTTIME=TTR/N)%>%arrange(desc(N))%>%print()#%>%
write.csv('p3.csv', row.names = F)
w%>% filter(Parent.Incident == "INC22306527")%>%
  select(Number,parentCulpable,systemShort,Affected.CI, Parent.Child,Parent.Incident,ttr)%>% 
  group_by(parentCulpable,systemShort)%>%
  summarize(N = n(),TTR=sum(ttr)) %>% 
  mutate(p = prop.table(N)*100)%>%
  mutate(CHTTIME=TTR/N)%>%arrange(desc(N))
w%>% filter(Parent.Incident == "INC24171934")%>%
  select(Number,systemShort,parentCulpable,Affected.CI, Parent.Child,Parent.Incident,ttr)%>% 
  group_by(parentCulpable,systemShort)%>%
  summarize(N = n(),TTR=sum(ttr)) %>% 
  mutate(p = prop.table(N)*100)%>%
  mutate(CHTTIME=TTR/N)%>%arrange(desc(N))
w%>% filter(Parent.Incident == "INC24423804")%>%
  select(Number,systemShort,parentCulpable,Affected.CI, Parent.Child,Parent.Incident,ttr)%>% 
  group_by(parentCulpable,systemShort)%>%
  summarize(N = n(),TTR=sum(ttr)) %>% 
  mutate(p = prop.table(N)*100)%>%
  mutate(CHTTIME=TTR/N)%>%arrange(desc(N))%>%print()#%>%
write.csv('p4.csv', row.names = F)
w%>% filter(Parent.Incident == "INC24267118")%>%
  select(Number,systemShort,parentCulpable,Affected.CI, Parent.Child,Parent.Incident,ttr)%>% 
  group_by(parentCulpable,systemShort)%>%
  summarize(N = n(),TTR=sum(ttr)) %>% 
  mutate(p = prop.table(N)*100)%>%
  mutate(CHTTIME=TTR/N)%>%arrange(desc(N))%>%print()#%>%
write.csv('p5.csv', row.names = F)
